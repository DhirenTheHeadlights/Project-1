...
void LandMassHandler::addLandMasses(int numLandMasses, float minDistBetweenLandmasses) {
	for (int i = 0; i < numLandMasses; i++) {
		...
		// If the number is between 6 and 8, generate a tree
		else if (randNum < 8) {
			landMass->createLandMass(LandMassType::Island, texture);
		}
		// If the number is between 8 and 10, generate a bush
		else {
		...
		if (randPos.has_value()) {
			...
			//std::cout << "Land mass generated at: " << randPos.value().x << ", " << randPos.value().y << std::endl;
		}
